...
# Railway hosted API Ã¢ÂÂ set CONCORDANCE_API_URL env var to enable ledger routing.
...
def validate_packet(packet: Dict[str, Any], now_epoch: Optional[int] = None) -> Dict[str, Any]:
    """Calibrate a packet against Source through all four gates: RED, FLOOR, BROTHERS, GOD.

    Concordance is a Wisdom Engine â alignment to Source, not mere validation.
    Each gate asks a different calibration question:
      RED     â Does this violate any absolute boundary?
      FLOOR   â Does this meet every protective constraint?
      BROTHERS â Have the right witnesses confirmed this?
      GOD     â Has sufficient time passed for Source confirmation?

    Attempts the Railway-hosted API first (writes to Evidence Ledger);
    falls back to local calibration if the API is unreachable.
