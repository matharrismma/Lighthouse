...
- [~] **Make the map the navigation spine** — hub pattern in progress: Codex hub (0a30b78) + gauntlet→engine-family hub-entry (0ca23ae) + media-center already hubs its family. Remaining hubs: Apothecary, Community, Write-Keep (Learn done: read.html hub-entry). Atlas-as-connection-map (statistics↔geometry↔logic) still the larger build. `[me]` L
