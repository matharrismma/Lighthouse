...
from .packet import ValidationResult as EngineResult, GateResult
from .validate import load_schema, validate_against_schema


_EXIT = {"PASS": 0, "REJECT": 1, "QUARANTINE": 2}
_ICON = {"PASS": "v", "REJECT": "x", "QUARANTINE": "~"}
...
def _format_summary(res: EngineResult, packet_path: Path) -> str:
    ...
    for gate in _GATE_ORDER:
        ...
        for r in results:
            if r.status in ("REJECT", "QUARANTINE"):
...
def main() -> None:
    ...
    v.add_argument("--no-verifiers", action="store_true")
