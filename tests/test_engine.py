...
def _run(name, packet, expected, now=FUTURE):
...
if __name__ == "__main__":
    ...
    _run("Math: undefined symbols → REJECT",
         base_packet(domain="mathematics", text="",
    ...
    _run("Math: flat packet with claims → PASS",
         base_packet(domain="mathematics", text="",
    ...
    _run("Physics with conservation → PASS",
         base_packet(domain="physics", text="",
                     conservation_checks={"energy": True}, units="SI"),
         "PASS")

    _run("Physics: no conservation field → REJECT",
    ...
    _run("Valid chemistry packet → PASS",
    ...
    _run("Valid biology packet -> PASS",
    ...
    _run("Biology: mass balance violation -> REJECT",
         base_packet(domain="biology", text="",
                     BIO_RED={"conservation": {"mass_balance": False}}),
         "REJECT")

    _run("Biology: causality missing -> REJECT",
         base_packet(domain="biology", text="",
                     BIO_RED={"conservation": {"mass_balance": True},
                              "causality": {"mechanism_specified": False}}),
         "REJECT")

    _run("Biology: decision-grade claim needs 2+ assay classes -> REJECT",
         base_packet(domain="biology", text="",
                     BIO_RED={"conservation": {"mass_balance": True}},
                     BIO_MEASUREMENT={"controls_included": True, "biological_replicates": 3,
    ...
    _run("Biology: insufficient replicates -> REJECT",
         base_packet(domain="biology", text="",
                     BIO_RED={"conservation": {"mass_balance": True}},
                     BIO_MEASUREMENT={"controls_included": True, "biological_replicates": 2}),
         "REJECT")

    _run("Biology: flat packet with claims -> PASS",
         base_packet(domain="biology", text="",
    ...
    _run("Chemistry: equilibrium_in_activities violation -> REJECT",
         base_packet(domain="chemistry", text="",
                     CHEM_SETUP={"temperature_K": 298.15, "system_boundary": "closed"},
    ...
    _run("Chemistry: hazardous with no safety notes -> REJECT",
         base_packet(domain="chemistry", text="",
                     CHEM_SETUP={"temperature_K": 298.15, "system_boundary": "closed",
    ...
    _run("Chemistry: limiting cases not checked -> REJECT",
         base_packet(domain="chemistry", text="",
                     CHEM_SETUP={"temperature_K": 298.15, "system_boundary": "closed",
                                 "ideality_assumption": "ideal"},
                     CHEM_RED={"mass_conserved": True, "charge_conserved": True},
    ...
    _run("Chemistry: state/path integrity violation -> REJECT",
         base_packet(domain="chemistry", text="",
                     CHEM_SETUP={"temperature_K": 298.15},
                     CHEM_RED={"mass_conserved": True, "state_path_integrity": False}),
         "REJECT")

    _run("Chemistry: MODEL_MISMATCH diagnostic -> REJECT",
         base_packet(domain="chemistry", text="",
                     CHEM_SETUP={"temperature_K": 298.15, "system_boundary": "closed",
                                 "ideality_assumption": "ideal"},
                     CHEM_RED={"mass_conserved": True, "charge_conserved": True},
    ...
    _run("CS: valid algorithm packet -> PASS",
    ...
    _run("Statistics: valid inference packet -> PASS",
    ...
    _run("Statistics: p-value misinterpretation -> REJECT",
         base_packet(domain="statistics", text="",
                     STAT_RED={"hypothesis_prespecified": True,
    ...
    _run("Statistics: effect size missing on significant result -> REJECT",
         base_packet(domain="statistics", text="",
                     STAT_RED={"hypothesis_prespecified": True,
                               "pvalue_interpreted_correctly": True},
    ...
    _run("Statistics: post-hoc hypothesis -> REJECT",
         base_packet(domain="statistics", text="",
    ...
    _run("Statistics: multiple comparisons not corrected -> REJECT",
         base_packet(domain="statistics", text="",
                     STAT_RED={"hypothesis_prespecified": True,
                               "pvalue_interpreted_correctly": True,
                               "effect_size_reported": True,
    ...
    _run("Statistics: distributional assumptions not tested -> REJECT (FLOOR)",
         base_packet(domain="statistics", text="",
                     STAT_RED={"hypothesis_prespecified": True,
                               "pvalue_interpreted_correctly": True,
                               "effect_size_reported": True},
    ...
    _run("Statistics: flat packet with claims -> PASS",
         base_packet(domain="statistics", text="",
    ...
    _run("Chemistry: balanced equation → PASS",
    ...
    _run("Chemistry: unbalanced equation → REJECT",
         base_packet(domain="chemistry",
                     CHEM_RED={"mass_conserved": True, "charge_balanced": True,
                               "elements_consistent": True, "phases_specified": True},
                     CHEM_SETUP={"reagents": ["H2", "O2"], "products": ["H2O"],
    ...
    _run("Chemistry: balanced redox with charges → PASS",
         base_packet(domain="chemistry",
                     CHEM_RED={"mass_conserved": True, "charge_balanced": True,
                               "elements_consistent": True, "phases_specified": True},
    ...
    _run("Chemistry: T<0 K in CHEM_VERIFY → REJECT",
         base_packet(domain="chemistry",
                     CHEM_RED={"mass_conserved": True, "charge_balanced": True,
                               "elements_consistent": True, "phases_specified": True},
    ...
    _run("Physics: F=ma dimensional → PASS",
    ...
    _run("Physics: F=mv dimensional MISMATCH → REJECT",
         base_packet(domain="physics",
                     conservation_checks={"dimensional_consistency": {"required": True, "verified": True}},
                     units="SI",
    ...
    _run("Physics: conservation within tol → PASS",
         base_packet(domain="physics",
                     conservation_checks={"dimensional_consistency": {"required": True, "verified": True}},
                     units="SI",
    ...
    _run("Physics: conservation drift → REJECT",
         base_packet(domain="physics",
                     conservation_checks={"dimensional_consistency": {"required": True, "verified": True}},
                     units="SI",
    ...
    _run("Mathematics: (x+1)^2 == x^2+2x+1 → PASS",
    ...
    _run("Mathematics: wrong derivative → REJECT",
         base_packet(domain="mathematics",
                     MATH_RED={
                         "well_formedness": {"symbols_defined": True, "quantifiers_scoped": True,
                                             "domains_declared": True},
    ...
    _run("Statistics: t-test p matches → PASS",
    ...
    _run("Statistics: Bonferroni rejection set wrong → REJECT",
         base_packet(domain="statistics",
                     STAT_INFERENCE={"test_specified": True, "assumptions_tested": True,
    ...
    _run("CS: linear scan with correct cases → PASS",
    ...
    _run("CS: while True without break → REJECT",
         base_packet(domain="computer_science",
                     CS_RED={"termination_proven": True, "memory_safety_addressed": True,
                             "no_undefined_behavior": True, "concurrency_correctness_addressed": True},
    ...
    _run("CS: wrong implementation → REJECT",
         base_packet(domain="computer_science",
                     CS_RED={"termination_proven": True, "memory_safety_addressed": True,
                             "no_undefined_behavior": True, "concurrency_correctness_addressed": True},
                     CS_COMPLEXITY={"case": "worst", "time": "O(1)", "space": "O(1)"},
    ...
    _run("CS: bubble sort claimed O(n**2) → PASS",
         base_packet(domain="computer_science",
                     CS_RED={"termination_proven": True, "memory_safety_addressed": True,
                             "no_undefined_behavior": True, "concurrency_correctness_addressed": True},
    ...
    _run("Biology: 4 replicates, 3 assay classes → PASS",
    ...
    _run("Biology: 2 replicates → REJECT",
         base_packet(domain="biology",
    ...
    _run("Biology: monotonic dose-response → PASS",
         base_packet(domain="biology",
    ...
    _run("Biology: non-monotonic without justification → REJECT",
         base_packet(domain="biology",
                     BIO_RED={"orthogonal_assays_used": ["functional_assay", "biochemical_assay"]},
                     artifacts={"protocol": "see methods"},
                     BIO_VERIFY={
    ...
    _run("Biology: underpowered sample size → REJECT",
         base_packet(domain="biology",
    ...
    _run("Governance: complete decision packet → PASS",
    ...
    _run("Governance: incomplete decision packet → REJECT",
         base_packet(domain="governance",
    ...
    _run("Governance: witness count mismatch → REJECT",
         base_packet(domain="governance",
    ...
    # ── Flat governance packet auto-wrap (regression for schema mismatch) ──
    print("\nFlat governance packet normalization:")

    # A flat packet with structured fields at the top level (no DECISION_PACKET
    # wrapper, no `text`) should auto-normalize and pass end-to-end.
    _run("Flat governance packet auto-wraps and PASSes",
         {
             "domain": "governance",
             "scope": "adapter",
             "created_epoch": PASS_TIME,
             ...
             "way_path": ("Sunday evening 30-minute review of prior week spend "
                          "and upcoming week plan"),
             "execution_steps": ["Print transactions", "Compare to targets",
                                 "Agree adjustments", "Log decisions"],
             "witnesses": ["Matt", "Spouse"],
             "scripture_anchors": ["Prov 27:23"],
         },
         "PASS")

    # ── wait_window_seconds raises (cannot lower) the scope floor ──
    print("\nWait window override semantics:")

    # Scope=adapter has a 3600s floor; packet's wait_window_seconds=0 must
    # NOT lower it. now_epoch is 100s past created → still QUARANTINE.
    _run("packet wait_window_seconds=0 cannot lower adapter floor",
         base_packet(wait_window_seconds=0),
         "QUARANTINE",
         now=PASS_TIME + 100)

    # Packet's wait_window_seconds RAISES the floor: 7200s > 3600s adapter
    # floor. now_epoch at +5000s is past adapter floor but not the packet's
    # raised floor → QUARANTINE.
    _run("packet wait_window_seconds=7200 raises adapter floor",
         base_packet(wait_window_seconds=7200),
         "QUARANTINE",
         now=PASS_TIME + 5000)

    # Same packet at +7300s is past the raised floor → PASS.
    _run("packet wait_window_seconds=7200 satisfied past raised floor",
         base_packet(wait_window_seconds=7200),
         "PASS",
         now=PASS_TIME + 7300)
