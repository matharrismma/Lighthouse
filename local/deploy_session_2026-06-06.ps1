...
# Ships everything built this session (commits 35780a2..27317e2):
...
#   - 7 build-queue gap closes (Atwater, inflation, atomic-mass, Clausius-
#     Clapeyron, speed-of-light test, classifier-no-autocorrect, scripture
#     quotation) + moon-landing reclassified to declined
#
# Review before running. Tailscale SSH + git push are yours.
# Note: covers this session's window. Earlier floor modules (calibre.py,
# nested_control.py) were shipped in a prior cycle; not re-sent here.
# ============================================================================
...
function Guard($label) {
  if ($LASTEXITCODE -ne 0) {
    Write-Host "  ✗ $label failed (exit $LASTEXITCODE) — stopping so the server isn't left half-updated." -ForegroundColor Red
    exit 1
  }
}
...
scp "$r\site\index.html" "$r\site\nh-shell.js" "$r\site\marketplace.html" "$r\site\innovation.html" "$r\site\gaps.html" "$r\site\workspace.html" "$r\site\family.html" "$r\site\learn-deep.html" "$r\site\codex-deep.html" "$r\site\take-part.html" "$r\site\sitemap.xml" "$r\site\sitemap_index.xml" "$srv`:~/Lighthouse/site/"
Guard "scp site"
...
Write-Host "  https://narrowhighway.com/marketplace.html   (by ZIP)" -ForegroundColor Green
...
Write-Host "(After a big deploy, purge the Cloudflare cache.)" -ForegroundColor DarkGray
